...
import net.kno3.season.roverruckus.solus.robot.acmedrive.TuningUtil;
...
@Config
@Autonomous
public class DriveFFTuningOpMode extends SolusAuto {
    ...
    public void main() {
        ...
        while (!isStopRequested()) {
            ...
            positionSamples.add(drive.roadrunner.getPoseEstimate().getX());

            drive.roadrunner.setVelocity(new Pose2d(power, 0.0, 0.0));
            drive.roadrunner.updatePoseEstimate();
        }
        drive.roadrunner.setVelocity(new Pose2d(0.0, 0.0, 0.0));
        ...
        if (fitAccelFF) {
            ...
            drive.roadrunner.setPoseEstimate(new Pose2d());
            drive.roadrunner.setVelocity(new Pose2d(MAX_POWER, 0.0, 0.0));
            while (!isStopRequested()) {
                double elapsedTime = clock.seconds() - startTime;
