package net.kno3.season.roverruckus.solus.program.dogecvtests;
...
public class CroppingGoldDetector extends DogeCVDetector {
    ...
    private Rect chosenYellowRect = null;
    ...
    private Mat croppingMat = new Mat();
    ...
    public CroppingGoldDetector() {
        super();
        this.detectorName = "Sampling Order Detector";
    }

    private static final int CROP_X1 = 50; //100
    private static final int CROP_X2 = 150;
    private static final int CROP_Y1 = 60;
    private static final int CROP_Y2 = 570;

    @Override
    public Mat process(Mat input) {

        // Crop the input first
        input.copyTo(croppingMat);
        input.release();

        croppingMat = croppingMat.submat(CROP_X1, CROP_X2, CROP_Y1, CROP_Y2);
        //Log.d("MATSIZE", "The mat size is " + croppingMat.cols() + " " + croppingMat.rows());

        // Copy input mat to working/display mats
        croppingMat.copyTo(displayMat);
        croppingMat.copyTo(workingMat);
        ...
        */
        ...
        double third = (CROP_Y2-CROP_Y1) / 3.0;

        if(chosenYellowRect != null) {

            if (chosenYellowRect.x < third) {
                currentOrder = CroppingGoldDetector.GoldLocation.LEFT;
            }

            if (chosenYellowRect.x >= third && chosenYellowRect.x < third * 2) {
                currentOrder = CroppingGoldDetector.GoldLocation.CENTER;
            }

            if (chosenYellowRect.x >= third * 2) {
                currentOrder = CroppingGoldDetector.GoldLocation.RIGHT;
            }
            isFound = true;
            lastOrder = currentOrder;

        }else{
            currentOrder = CroppingGoldDetector.GoldLocation.UNKNOWN;
            isFound = false;
        }
    ...
    public double getGoldRectX() {
        return chosenYellowRect.x;
    }
}
