package net.kno3.season.roverruckus.solus.robot;

import com.qualcomm.robotcore.eventloop.opmode.OpMode;

import net.kno3.robot.Robot;
import net.kno3.util.SimpleColor;

/**
 * Created by robotics on 10/28/2017.
 */

public class Solus extends Robot {
    public static final String DRIVE_FL_KEY = "frontleft";
    public static final String DRIVE_FR_KEY = "frontright";
    public static final String DRIVE_RL_KEY = "rearleft";
    public static final String DRIVE_RR_KEY = "rearright";

    public static final String HOPPER_FLIPPER_KEY = "Hopper Flipper";
    public static final String HOPPER_ROOF_KEY = "Hopper Roof";
    public static final String HOPPER_SWEEPER_KEY = "Hopper Sweeper";
    public static final String HANG_SERVO_KEY = "Hang Servo";
    public static final String LAND_SERVO_KEY  = "Land";
    public static final String HOPPER_LIFT_RIGHT_KEY = "Hopper Motor Right";
    public static final String HOPPER_LIFT_LEFT_KEY = "Hopper Motor Left";

    public static final String SILVER_SORTER_KEY = "Silver Sorter";
    public static final String GOLD_SORTER_LEFT_KEY = "Gold Sorter Left";
    public static final String GOLD_SORTER_RIGHT_KEY = "Gold Sorter Right";
    public static final String INTAKE_FLIPPER_LEFT_KEY = "Intake Flipper Left";
    public static final String INTAKE_FLIPPER_RIGHT_KEY = "Intake Flipper Right";
    public static final String INTAKE_SLIDE_MOTOR_KEY = "Intake Slide Motor";
    public static final String TEAM_MARKER_DUMPER_KEY = "Team Marker Dumper";
    public static final String COLLECTOR_KEY = "Collector";

    public static final String RANGE_KEY = "sensor_range";
    public static final String IMU_KEY = "imu";

    public static SimpleColor SIDE;



    public Solus(OpMode opMode) {
        super(opMode);

        putSubSystem("drive", new DriveSystem(this));
        putSubSystem("hopper", new HopperSystem(this));
        putSubSystem("intake", new IntakeSystem(this));
        putSubSystem("led", new LEDSystem(this));

    }
}